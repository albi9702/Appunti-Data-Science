...
Each document is represented by a vector of words, such as binary representation. Another representation is the frequency of the words or the weighted representation, tf-idf.
...
For simple scenarios it is possible create a $k$-dimensional mapping for a simple example vocabulary y manually choosing contextual dimensions that make sense. This is not a simple task, in particular manual assignment of vector would be impossibly. The Distributional Hypothesis aims to represent each words by some context.
...
$$PMI(x, y) = \log_2 \frac{\mathbb{P}(x, y)}{\mathbb{P}(x)\mathbb{P}(y)} \in (- \infty, + \infty)$$

where $x$ is the target word and $y$ the context. To have better estimation, only positive estimation are taken:

$$PPMI(x, y) = \max [PMI(x,y),0]$$

$f_{i, j}$ is the number of time the word $w_i$ appears in the context $c_j$. It is used to compute
...
$$P_{\alphs}(c) = \frac{\text{count}(c)^{\alpha}}{\sum_c \text{count}^{\alpha}}$$
